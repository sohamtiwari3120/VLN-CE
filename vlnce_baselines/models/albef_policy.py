...
class ALBEFNet(Net):
    ...
    def __init__(
        ...
        self.instr_proj = nn.Linear(768, 768)
        ...
        self.vit_out_shape = 197 #* 768
        self.vis_proj = nn.Linear(768, 768)
    ...
    def forward(
        ...
        # for i in range(60):
        #     plt.imshow(observations['rgb'][i].cpu().numpy(), interpolation='nearest')
        #     plt.show()
        #     plt.close()
        ...
        # RGB observation has shape (batch, 224, 224, 3)
        rgb_observations = observations["rgb"].permute(0, 3, 1, 2).cpu()
        rgb_observations_list = list(torch.split(rgb_observations, rgb_observations.shape[0], dim=0)[0])
        rgb_fe = self.vit_feature_extractor(rgb_observations_list, return_tensors='pt')
        rgb_fe['pixel_values'] = rgb_fe['pixel_values'].to('cuda')
        with torch.no_grad():
            rgb_out = self.vit_model(**rgb_fe)
        
        # RGB output (batch_size, 197, 768)
        rgb_embedding = torch.flatten(rgb_out.last_hidden_state, 2)
        # resnet encoding 
        # rgb_embedding = self.rgb_encoder(observations)
        # rgb_embedding = torch.flatten(rgb_embedding, 2)
        # ------------
        
        # [CLS] tokens for instruction and image with shape (batch_size, hidden_size=768)
        # t_cls = observations['rxr_instruction'][:, 0, :]
        # i_cls = rgb_embedding[:, 0, :]
        ...
        rgb_k, rgb_v = torch.split(
            ...
            # self.rgb_kv(rgb_embedding), self._hidden_size, dim=1
        )
