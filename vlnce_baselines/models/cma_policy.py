...
class CMAPolicy(ILPolicy):
    ...
    def act(
        ...
        # print(f"PROBS", distribution.probs, action, torch.special.entr(distribution.probs))

        return action, rnn_states, torch.distributions.Categorical(probs=distribution.probs).entropy(), distribution.probs
